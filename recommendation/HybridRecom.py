...
class HybridRecom:
    def __init__(self, path1, path2, path3):
        ...
        self.item_result = pd.read_csv('item_result.csv', encoding="gbk")
    ...
    def recommend(self):
        ...
        for i in self.row_tr:
            ...
            # 建立节目相似度字典
            pro_dict = {}
            item_based_user = self.item_result[self.item_result[u'设备号'] == i]
            item_based_pro = list(item_based_user[u"节目"])
            for item in item_based_pro:
                if item in pro_set:
                    item_based_i = item_based_user[item_based_user[u'节目'] == item]
                    item_based_rate = list(item_based_i[u"推荐度"])
                    pro_dict[item] = item_based_rate[0]
            for j in pro:
                v = u[u[u"节目"] == j]
                rating = v[u"喜爱度"]
...
hr = HybridRecom(path1, path2, path3)
hr.main()
end = time.clock()
print u"任务已完成"
